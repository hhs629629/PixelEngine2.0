...
void Chunk::read() {
    ...
    if(myfile.is_open()) { // 파일이 열려있으면 
        ...
        myfile.read((char*)&this->generationPhase, sizeof(int8_t));//파일을 읽는다
        ...
        if(readBuf == NULL) throw std::runtime_error("Failed to allocate memory for Chunk readBuf."); // 압축된 파일에 대한 다양한 정보를 수집하여 압축 해제를 할 준비를 함
...
void Chunk::write(MaterialInstance* tiles, MaterialInstance* layer2, Uint32* background) { // 청크 정보를 파일에 쓰는 함수
    ...
    delete[] buf;
    myfile.write((char*)compressed_data2, compressed_data_size2); // 청크 정보들을 적절한 순서로 파일에 기록
...
std::vector<std::string> string_split(std::string s, const char delimiter) { // 문자열 분할 함수
    size_t start = 0;
